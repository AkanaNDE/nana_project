...
class Maho_node(Node):
    motor1Speed : float = 0


    def __init__(self):
        super().__init__("Maho_node")
        self.motor1Speed = 0.0


        self.send_robot_speed = self.create_publisher(
            Twist, "/nana/cmd_maho/rpm", qos_profile=qos.qos_profile_system_default
        )

        self.create_subscription(
            Twist, '/nana/cmd_maho', self.cmd_vel, qos_profile=qos.qos_profile_system_default
        )
