...
void mddMotorSet(int pinA, int pinB, int chA, int chB, int speed, bool forward) {
  ...
  if (forward) { ledcWrite(chA, speed); ledcWrite(chB, 0); }
...
// ── differential: ลดล้อข้างที่ต้องเลี้ยว ไม่บวกอีกข้าง ──
void driveWithTurn(int base_pwm, float turn_sign, int turn_pwm, bool forward_dir) {
  int delta     = (int)(0.4f * turn_pwm);
  int left_pwm  = base_pwm;
  int right_pwm = base_pwm;

  if (turn_sign > 0) {
    // หันซ้าย → ลดล้อซ้าย
    left_pwm  = constrain(base_pwm - delta, 0, 1023);
  } else {
    // หันขวา → ลดล้อขวา
    right_pwm = constrain(base_pwm - delta, 0, 1023);
  }
...
void driveFromTwist(float x, float y, float z) {
  ...
  // ── เดินตรง (z == 0) → base_pwm เท่ากันสองข้าง ──
  if (forward != 0.0f && z == 0.0f) {
    bool fwd = (forward > 0.0f);
    driveLeftSide(base_pwm, fwd);
    driveRightSide(base_pwm, fwd);
    return;
  }

  // เดินพร้อมเลี้ยว
  if (forward != 0.0f && z != 0.0f) {
    driveWithTurn(base_pwm, (z > 0) ? +1.0f : -1.0f, turn_pwm, forward > 0.0f);
    return;
  }
...
void setup() {
  ...
  ledcSetup(CH_M1A, PWM_FREQ, PWM_RES); ledcSetup(CH_M1B, PWM_FREQ, PWM_RES);
  ledcSetup(CH_M2A, PWM_FREQ, PWM_RES); ledcSetup(CH_M2B, PWM_FREQ, PWM_RES);
  ledcSetup(CH_M3A, PWM_FREQ, PWM_RES); ledcSetup(CH_M3B, PWM_FREQ, PWM_RES);
  ledcSetup(CH_M4A, PWM_FREQ, PWM_RES); ledcSetup(CH_M4B, PWM_FREQ, PWM_RES);
  ...
  cmd_msg_in.linear.x  = 0.0; cmd_msg_in.linear.y  = 0.0; cmd_msg_in.linear.z  = 0.0;
  cmd_msg_in.angular.x = 0.0; cmd_msg_in.angular.y = 0.0; cmd_msg_in.angular.z = 0.0;
  ...
  RCCHECK(rclc_executor_add_subscription(
    &executor, &cmd_subscriber, &cmd_msg_in, &cmd_callback, ON_NEW_DATA
  ));

  last_cmd_ms         = millis();
  last_encoder_pub_ms = millis();

  publish_ack("ESP READY (differential PID mode)");
}
...
void loop() {
  ...
  if (!got_first_cmd) { stopAll(); return; }
