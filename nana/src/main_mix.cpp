...
Servo servo1;
Servo servo2;
Servo servo3;                    // ← Stage 7
const int servo1_pin = 27;
const int servo2_pin = 26;
const int servo3_pin = 25;       // ← Stage 7
...
rcl_subscription_t sub_servo_cmd;   // ← Stage 7

rcl_publisher_t publisher;

std_msgs__msg__Bool plant_msg;
std_msgs__msg__Bool plant2_msg;
std_msgs__msg__Bool servo_cmd_msg;  // ← Stage 7
...
// ── Stage 7: รับคำสั่งจาก orchestrator ──
void servo_cmd_callback(const void * msgin)
{
  const std_msgs__msg__Bool * msg = (const std_msgs__msg__Bool *)msgin;
  if (msg->data)
  {
    // True → หมุน 30 องศา
    servo3.write(150);
    Serial.println("[Servo3] → 30 deg");
  }
  else
  {
    // False → กลับ 0 องศา
    servo3.write(175);
    Serial.println("[Servo3] → 0 deg");
  }
}
...
void setup()
{
  ...
  // subscribe /servo_cmd  ← Stage 7
  rclc_subscription_init_default(
    &sub_servo_cmd, &node,
    ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Bool),
    "/servo_cmd");

  // publisher /Finishplant
  rclc_publisher_init_default(
    &publisher, &node,
    ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Bool),
    "/Finishplant");

  // executor — เพิ่มเป็น 3 subscriptions
  rclc_executor_init(&executor, &support.context, 3, &allocator);

  rclc_executor_add_subscription(
    &executor, &sub_plant, &plant_msg,
    &plant_callback, ON_NEW_DATA);

  rclc_executor_add_subscription(
    &executor, &sub_plant2, &plant2_msg,
    &plant2_callback, ON_NEW_DATA);

  rclc_executor_add_subscription(
    &executor, &sub_servo_cmd, &servo_cmd_msg,  // ← Stage 7
  ...
  servo3.attach(servo3_pin);   // ← Stage 7
  servo3.write(175);             // เริ่มต้นที่ 0 องศา
...
void loop()
{
  ...
  if (start_sequence)
  {
    ...
    stepper_run(HIGH, 1500);
    stepper_run(LOW,  500);
    servo1.write(120); delay(1000);
    servo1.write(150); delay(1000);
    stepper_run(HIGH, 500);
    servo1.write(160); delay(1000);
    servo1.write(110); delay(1000);
    stepper_run(LOW,  1400);
    servo1.write(180); delay(1000);
    servo2.write(120); delay(1500);
    servo2.write(70);  delay(1000);
